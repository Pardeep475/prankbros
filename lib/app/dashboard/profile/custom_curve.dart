...
class MyWaveClipperOne extends CustomClipper<Path> {
  bool reverse;
  bool flip;

  MyWaveClipperOne({this.reverse = false, this.flip = false});
  ...
  Path getClip(Size size) {
    if (!reverse && !flip) {
      ...
      Offset firstEndPoint = Offset(size.width * .5, size.height - 20);
      Offset firstControlPoint = Offset(size.width * .25, size.height - (-20));
      Offset secondEndPoint = Offset(size.width, size.height);
      Offset secondControlPoint = Offset(size.width * .75, size.height - 50);
